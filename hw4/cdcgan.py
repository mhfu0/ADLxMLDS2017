...
def Discriminator(x, y, reuse=False):
    ...
    with tf.variable_scope('d_net') as sc:
        ...
        fc = tc.layers.fully_connected(fc, 96, weights_initializer=tf.random_normal_initializer(stddev=0.02), activation_fn=None)
        fc = leaky_relu(fc)
        fc = tf.nn.dropout(fc, 0.3)
        
        fc_y = tf.concat([fc, y], axis=1)
        print('fc_y.shape =',fc_y.shape)
...
y = tf.placeholder(tf.float32, shape=[None, 24])
z = tf.placeholder(tf.float32, shape=[None, 96])

w = tf.placeholder(tf.float32, shape=[None, 24]) # wrong label
...
with tf.Session(config=config) as sess:
    ...
    if not TRAIN:
        ...
        test_rand = np.random.uniform(0., 1., size=[5, 96])
        ...
        for i in range(5)
            scipy.misc.imsave('result/result_%d.jpg' % i, np.squeeze(data[i]))
        ...
        for ep in range(200):
            for i in range(500):
                ...
                rand = np.random.uniform(0., 1., size=[BATCH_SIZE, 96])
            ...
            # output result
            test_rand = np.random.uniform(0., 1., size=[1, 96])
            test_label = np.zeros(24)
            test_label[np.array([5,19])] = 1.
